	      ...
	      <?php if ($this->params->get('enable_samlogin', 1)) { ?>   
               <div class="samloginSAMLLoginBlock">
					<h2 class="samloginServicesMessage"><?php echo $this->params->get('ssoLoginTitle',"Federated Login");?></h2>
                                        <form class="samloginBlock form-inline " action="<?php echo $this->ssoLink; ?>" method="POST">
                                              <div class="form-group">
                                                  <script>
                                                  jQuery(document).ready(function(){
                                                            jQuery('.validate-emailcustom').on("keyup", function (e) {
                                                                   var regex = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i;
                                                                   var valid= regex.test(jQuery(this).val()); 
                                                                   if (!valid){
                                                                       jQuery(".ssoEmailGoButton").attr("disabled","true");
                                                                   }else{
                                                                       jQuery(".ssoEmailGoButton").attr("disabled",null);
                                                                   }
                                                            });
                                                            });
                                                  </script>
                                                  <input type="text" class="form-control required validate-emailcustom" name="useremail" value="" placeholder="Insert your email here">
                                                    </div>
                                                 <div class="form-group">
                                                     <button type=submit class="ssoEmailGoButton <?php echo $this->params->get('loginButtonClasses',"btn btn-primary uk-button uk-button-primary");?>" href="<?php echo $this->ssoLink; ?>">
							<i></i>
